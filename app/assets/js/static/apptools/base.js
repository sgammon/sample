...
(function() {
  var AnimationInterface, AppTools, AppToolsDriver, AppToolsException, CoreAPI, CoreAgentAPI, CoreDevAPI, CoreDriver, CoreEventsAPI, CoreException, CoreInterface, CoreModelAPI, CoreObject, CoreRPCAPI, CoreRenderAPI, CoreServicesAPI, CoreStorageAPI, DOMInterface, Driver, GoogleAnalytics, Integration, Interface, Key, KeyEncoder, ListField, LocalStorageDriver, LocalStorageEngine, Model, ModelException, NativeSocket, NativeXHR, QueryInterface, RPC, RPCAPI, RPCContext, RPCEnvelope, RPCErrorResponse, RPCInterface, RPCPromise, RPCRequest, RPCResponse, RealtimeDriver, RenderException, RenderInterface, RequestEnvelope, ResponseEnvelope, ServiceLayerDriver, SessionStorageDriver, SessionStorageEngine, SimpleKeyEncoder, SocketCommands, SocketInterface, SocketProtocol, SocketState, SocketVocabulary, StorageAdapter, StorageDriver, Template, TemplateAPI, TemplateLoader, TransportInterface, Util, boot, i, __apptools_preinit_bootlist, _i, _j, _len, _len1, _ref, _simple_key_encoder,
  ...
  */
  ...
  this.SocketInterface = SocketInterface = (function(_super) {

    __extends(SocketInterface, _super);

    function SocketInterface() {
      return SocketInterface.__super__.constructor.apply(this, arguments);
    }

    SocketInterface.prototype.capability = 'socket';

    SocketInterface.prototype.parent = TransportInterface;

    SocketInterface.prototype.required = ['factory', 'open', 'send', 'close', 'message'];

    SocketInterface.prototype.open = function() {};

    SocketInterface.prototype.send = function() {};

    SocketInterface.prototype.close = function() {};

    SocketInterface.prototype.factory = function() {};

    SocketInterface.prototype.message = function() {};
  ...
  this.SocketState = SocketState = (function(_super) {
    var CLOSED, ERROR, OPEN, SLEEP;

    __extends(SocketState, _super);

    function SocketState() {
      return SocketState.__super__.constructor.apply(this, arguments);
    }
  ...
  this.SocketCommands = SocketCommands = (function(_super) {
    var ALLOW, AUTH, CLOSE, DENY, INIT, NOTIFY, PING, PONG, PUBLISH, REQUEST, RESPONSE, SUBSCRIBE, SYNC, WAIT;

    __extends(SocketCommands, _super);

    function SocketCommands() {
      return SocketCommands.__super__.constructor.apply(this, arguments);
    }
  ...
  this.NativeSocket = NativeSocket = (function(_super) {

    __extends(NativeSocket, _super);

    NativeSocket.prototype.id = 0;

    NativeSocket.prototype.uuid = null;

    NativeSocket.prototype.name = null;

    NativeSocket.prototype.host = null;

    NativeSocket.prototype.state = SocketState.CLOSED;

    NativeSocket.prototype.config = {};

    NativeSocket.prototype.events = {};

    NativeSocket.prototype.socket = null;

    NativeSocket.prototype.request = null;

    NativeSocket.prototype.protocols = [];

    function NativeSocket(config, events, socket) {
      var proxy,
        _this = this;
      this.config = config;
      this.events = events != null ? events : {};
      this.socket = socket != null ? socket : WebSocket;
      this.transmit = __bind(this.transmit, this);

      this.get_header = __bind(this.get_header, this);

      this.set_header = __bind(this.set_header, this);
      ...
      proxy = {
        open: function(event) {
          _this.internal.state(SocketState.OPEN);
          if (_this.events.open != null) {
            return _this.events.open(event, _this.socket);
          }
        },
        error: function(event) {
          _this.internal.state(SocketState.ERROR);
          if (_this.events.error != null) {
            return _this.events.error(event, _this.socket);
          }
        },
        message: function(event) {
          if (_this.events.message != null) {
            return _this.events.message(event.data, event, _this.socket);
          }
        },
        close: function(event) {
          _this.internal.state(SocketState.CLOSED);
          if (_this.events.close != null) {
            return _this.events.close(event, _this.socket);
          }
        }
      };
      this.internal = {
        bind: function() {
          _this.socket.onopen = proxy.open;
          _this.socket.onclose = proxy.close;
          _this.socket.onmessage = proxy.message;
          _this.socket.onerror = proxy.error;
          return _this;
        },
        send: function() {
          var args, _ref;
          args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
          return (_ref = _this.socket).send.apply(_ref, args);
        },
    ...
    NativeSocket.prototype.add_protocol = function(object) {
      var p, _j, _len1, _ref;
      _ref = this.protocols;
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        p = _ref[_j];
      ...
      this.protocols.unshift(object);
      return this;
    };

    NativeSocket.prototype.list_protocols = function() {
      return this.protocols;
    };

    NativeSocket.prototype.remove_protocols = function(name) {
      var i, index, p, _j, _len1, _ref;
      index = null;
      _ref = this.protocols;
    ...
    NativeSocket.prototype.open = function(url) {
      var protocol, socket, transport;
      if (url == null) {
        url = null;
      }
      if (!(url != null) && !this.config.host && !(this.config.endpoint != null)) {
        throw "Must provide a host or socket URL.";
      } else {
        if (url != null) {
          socket = this.socket = new this.socket(url, (function() {
            var _j, _len1, _ref, _results;
            _ref = this.protocols;
            _results = [];
            for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
              protocol = _ref[_j];
              _results.push(protocol.name);
            }
            return _results;
          }).call(this));
        } else {
          if (this.config.endpoint != null) {
            socket = this.socket = new this.socket(this.config.endpoint, (function() {
              var _j, _len1, _ref, _results;
              _ref = this.protocols;
              _results = [];
              for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
            ...
            if (this.config.secure != null) {
              transport = this.config.secure ? 'wss' : 'ws';
            } else {
              transport = 'ws';
            }
            socket = this.socket = new this.socket([transport, this.config.host].join('://'), (function() {
              var _j, _len1, _ref, _results;
              _ref = this.protocols;
              _results = [];
              for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
    ...
    NativeSocket.prototype.establish = NativeSocket.prototype.open;

    NativeSocket.prototype.close = function(graceful) {
      if (graceful == null) {
        graceful = true;
      }
      if (graceful) {
        return this.send(SocketCommands.CLOSE);
      } else {
        return this.socket.close();
      }
    };

    NativeSocket.prototype.destroy = NativeSocket.prototype.open;

    NativeSocket.prototype.send = function(command, data) {
      if (data == null) {
        data = null;
      }
      if (typeof command === 'number') {
        return this.transmit(this.serialize(command, data));
      } else {
        if (!(data != null)) {
          return this.transmit(command);
        } else {
          return this.transmit(this.serialize(SocketCommands[command], data));
        }
      }
    };

    NativeSocket.prototype.serialize = function(command, data) {
      var cmd, cmdlist, i, _j, _len1;
      if (data == null) {
        data = null;
      }
      ...
      cmdlist = [];
      for (i = _j = 0, _len1 = command.length; _j < _len1; i = ++_j) {
        cmd = command[i];
        switch (cmd) {
          case SocketCommands.PING:
            cmdlist.push(0);
            break;
          case SocketCommands.PONG:
            cmdlist.push(1);
            break;
          case SocketCommands.INIT:
            cmdlist.push(this.identify());
            break;
          case SocketCommands.REQUEST:
            cmdlist.push(data[i].payload());
            break;
          default:
            cmdlist.push(data[i]);
        }
    ...
    NativeSocket.prototype.set_header = function(name, value) {
      this.config.headers[name] = value;
      return this;
    };

    NativeSocket.prototype.get_header = function(name) {
      if (this.config.headers[name] != null) {
    ...
    NativeSocket.prototype.transmit = function(body) {
      var payload;
      payload = {
        id: this.id++,
        headers: {},
      ...
      this.payload.headers = _.extend({}, this.payload.headers, this.config.headers);
      return this.internal.send(JSON.stringify(payload));
    };
  ...
  this.SocketVocabulary = SocketVocabulary = (function(_super) {

    __extends(SocketVocabulary, _super);

    function SocketVocabulary() {
      return SocketVocabulary.__super__.constructor.apply(this, arguments);
    }

    return SocketVocabulary;

  })(CoreObject);

  this.SocketProtocol = SocketProtocol = (function(_super) {

    __extends(SocketProtocol, _super);

    function SocketProtocol() {
      return SocketProtocol.__super__.constructor.apply(this, arguments);
    }

    SocketProtocol.name = 'APTLS_V1';

    SocketProtocol.state = SocketState;

    SocketProtocol.commands = SocketCommands;

    SocketProtocol.vocabulary = SocketVocabulary;

    SocketProtocol.prototype.install = function(window, i) {
      window.__apptools_preinit.abstract_base_classes.push(i);
      window.NativeSocket.prototype.add_protocol(i);
      return i;
    };

    SocketProtocol.prototype.get_command = function(id) {
      var cmd, sentinel, _ref;
      _ref = this.commands;
      for (cmd in _ref) {
        sentinel = _ref[cmd];
        if (id === sentinel) {
    ...
    SocketProtocol.prototype.pack = function(command, frame) {
      return this.encode(this.serialize(command, frame));
    };

    SocketProtocol.prototype.unpack = function(raw) {
      return this.deserialize(this.decode(raw));
    };
  ...
  this.RealtimeDriver = RealtimeDriver = (function(_super) {

    __extends(RealtimeDriver, _super);

    RealtimeDriver.prototype.name = 'apptools';

    RealtimeDriver.prototype["native"] = true;

    RealtimeDriver.prototype["interface"] = [SocketInterface];

    function RealtimeDriver(apptools) {
      var _this = this;
      this.internal = {
        impl: NativeSocket,
        state: SocketState,
      ...
      this.rpc = {
        factory: function(context) {
          return _this.internal.acquire(context);
        },
        fulfill: function(sock, request, dispatch) {
          return _this.rpc.factory();
        },
        open: function() {},
        send: function() {},
        close: function() {},
  ...
  _ref = [TransportInterface, RPCInterface, SocketInterface, NativeSocket, SocketProtocol, NativeXHR, RPCPromise, ServiceLayerDriver];
  for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
    i = _ref[_j];
  ...
  GoogleAnalytics = (function(_super) {
    ...
    GoogleAnalytics.mount = 'analytics';

    GoogleAnalytics["export"] = 'private';

    GoogleAnalytics.events = ['ANALYTICS_INIT', 'ANALYTICS_PUSH', 'ANALYTICS_EVENT', 'ANALYTICS_READY', 'ANALYTICS_CONFIG', 'ANALYTICS_SETVAR', 'ANALYTICS_TRACK_EVENT', 'ANALYTICS_TRACK_SOCIAL', 'ANALYTICS_TRACK_TIMING', 'ANALYTICS_TRACK_PAGEVIEW', 'ANALYTICS_TRACK_CAMPAIGN', 'ANALYTICS_TRACK_TRANSACTION', 'ANALYTICS_TRANSACTION_NEWITEM', 'ANALYTICS_TRANSACTION_COMPLETE'];

    function GoogleAnalytics(apptools, window) {
      var _this = this;
      this.state = {
        vars: {},
        agent: {},
      ...
      this._init = function(apptools) {};
      this.internal = {
        initialize: function(queue, tracker, config, event) {
          var args, category, clockpoint, installed_trackers, k, key, label, now, sample_rate, start_time, value, variable, _k, _len2, _ref1, _ref2;
          if (config == null) {
            config = null;
          }
          if (event == null) {
          ...
          if (!_this.state.initialized) {
            ...
            if ((!(config != null)) && (window._gac != null)) {
              config = window._gac;
            }
            _this.state.config = _.extend(true, {}, _this.state.config, config);
            $.apptools.dev.verbose('Analytics', 'Merged analytics config.', config);
            _this.internal.bind_events();
            installed_trackers = [
              (function() {
                var _k, _len2, _ref1, _results;
                _ref1 = this.state.config.account_ids;
                _results = [];
                for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
                  k = _ref1[_k];
                  _results.push(k);
            ...
            _this.state.initialized = installed_trackers.length;
            _this.state._ga.queue = queue;
            _this.state._ga.tracker = tracker;
            _ref1 = _this.state.config.account_ids;
            for (key in _ref1) {
              value = _ref1[key];
              _this.internal.provision_tracker(key, value);
              if (_this.state.config.multitrack) {
                continue;
              } else {
                break;
              }
            }
            if ((window.__clock != null) && window.__clock.ts.length > 0) {
              _ref2 = window.__clock.ts;
              for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
                clockpoint = _ref2[_k];
                if (_.is_array(clockpoint)) {
                  now = clockpoint[0], args = clockpoint[1];
                  category = args[0], variable = args[1], start_time = args[2], label = args[3], sample_rate = args[4];
                  _this.track.timing(category, variable, Math.floor(now) - Math.floor(start_time), label, sample_rate);
                }
              }
            }
          }
        ...
        bind_events: function(event) {
          _.bind('ANALYTICS_INIT', function(tracker) {
            var boot_cmd_list;
            $.apptools.dev.verbose('Analytics', 'Tracker initialized.', tracker);
            boot_cmd_list = [];
            if (_this.state.config.linker) {
              boot_cmd_list.push(['_setAllowLinker', true]);
            }
            boot_cmd_list.push(['_setAccount', _this.state.config.account_ids[tracker.name]]);
            if ((event != null) && event.srcElement.hasAttribute('data-hostname')) {
              boot_cmd_list.push(['_setDomainName', event.srcElement.getAttribute('data-hostname')]);
            }
            if (_this.state.config.samplerate != null) {
              boot_cmd_list.push(['_setSiteSpeedSampleRate', _this.state.config.samplerate]);
            } else {
              _this.state.config.samplerate = 30;
            }
            _this.internal.push_command(boot_cmd_list, tracker.name);
            _this.state.initialized--;
            if (_this.state.initialized === 0) {
              _this.state.initialized = true;
              _.trigger('ANALYTICS_READY', _this.state.trackers);
            }
          });
          _.bind('ANALYTICS_PUSH', function() {
            var args, method;
            method = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
            $.apptools.dev.verbose('Analytics', 'Pushing tracker command.', method, args);
            args.unshift(method);
            _this.internal.push_command(args);
          });
          _.bind('ANALYTICS_READY', function(trackers) {
            $.apptools.dev.verbose('Analytics', 'Analytics is READY to receive events.', 'config:', _this.state.config, 'trackers:', trackers);
            return _.trigger('ANALYTICS_PUSH', ['_trackPageview']);
          });
          _.bridge(['ANALYTICS_SETVAR', 'ANALYTICS_TRACK_EVENT', 'ANALYTICS_TRACK_SOCIAL', 'ANALYTICS_TRACK_TIMING', 'ANALYTICS_TRACK_PAGEVIEW', 'ANALYTICS_TRACK_CAMPAIGN', 'ANALYTICS_TRACK_TRANSACTION', 'ANALYTICS_TRANSACTION_NEWITEM', 'ANALYTICS_TRANSACTION_COMPLETE'], ['ANALYTICS_PUSH'], function(args, source) {
            var command_spec, key, value, _k, _len2;
            command_spec = [_this.state._ga.bindings[source]];
            if (_.is_array(args) && args.length > 1) {
              ...
              if ((_.is_object(args) && !_.is_array(args)) || (args.length === 1 && _.is_object(args[0]))) {
                if (!_.is_object(args)) {
                  args = args[0];
                }
                for (key in args) {
          ...
          _.bridge(['ANALYTICS_INIT', 'ANALYTICS_PUSH', 'ANALYTICS_READY'], ['ANALYTICS_EVENT'], function(args, source) {
            var event_spec;
          ...
          return _this.mappings;
        },
        provision_tracker: function(name, account) {
          _this.state.trackers[name] = _this.state._ga.tracker._createTracker(account, name);
          $.apptools.events.trigger('ANALYTICS_INIT', {
            name: name,
            tracker: _this.state.trackers[name]
          });
          return _this.state.trackers[name];
        },
        push_command: function(command, tracker) {
          var cmd_statement, command_spec, name, t, targets, touched_trackers, _k, _l, _len2, _len3, _ref1;
          if (tracker == null) {
            tracker = null;
          }
          $.apptools.dev.verbose('Analytics', 'Pushing command.', command, tracker);
          if (tracker === null) {
            targets = _.extend({}, _this.state.trackers);
          } else {
            if (_.is_array(tracker)) {
              for (_k = 0, _len2 = tracker.length; _k < _len2; _k++) {
                t = tracker[_k];
                targets[t] = _this.state.trackers[t];
              }
            } else {
              targets = {};
              targets[tracker] = _this.state.trackers[tracker];
            }
          }
          $.apptools.dev.verbose('Analytics', 'Resolved target trackers.', targets);
          touched_trackers = 0;
          for (name in targets) {
            tracker = targets[name];
            command_spec = [];
            if (_.is_array(command)) {
              if (_.is_string(command[0])) {
                ...
                for (_l = 0, _len3 = command.length; _l < _len3; _l++) {
                  cmd_statement = command[_l];
                  if (!_.is_string(cmd_statement[0])) {
                    continue;
                  } else {
            ...
            (_ref1 = _this.state._ga.queue).push.apply(_ref1, command_spec);
      ...
      this.config = {
        flash: function(enable) {
          if (enable == null) {
            enable = null;
          }
          if (enable != null) {
            return _this.state._ga.tracker._setDetectFlash(enable);
          }
          return _this.state._ga.tracker._getDetectFlash();
        },
        title: function(enable) {
          ...
          if (enable != null) {
            return _this.state._ga.tracker._setDetectTitle(enable);
          }
          return _this.state._ga.tracker._getDetectTitle();
        },
        client: function(enable) {
          if (enable == null) {
            enable = null;
          }
          if (enable != null) {
            return _this.state._ga.tracker._setClientInfo(enable);
          }
          return _this.state._ga.tracker._getClientInfo();
        },
        anonymize: function(enable) {
          if (enable == null) {
            enable = null;
          }
          if (enable != null) {
            _this.state.config.anonymize = enable;
          }
          return _this.state.config.anonymize;
        },
        samplerate: function(value) {
          ...
          if (value != null) {
            _this.state.config.samplerate = value;
          }
          return _this.state.config.samplerate;
        }
      };
      this.vars = {
        set: function(slot, name, value, scope) {
          if (scope == null) {
            scope = 'page';
          }
          if (_.is_string(scope)) {
            scope = _.indexOf(_this.state.config._ga.scopes, scope);
          }
          _.trigger.apply(_, ['ANALYTICS_SETVAR'].concat(__slice.call([slot, name, value, scope])));
          return _this;
        }
      };
      this.track = {
        event: function(category, action, label, value, non_interaction) {
          var _command_spec;
          if (label == null) {
            label = null;
          }
          ...
          _.trigger.apply(_, ['ANALYTICS_TRACK_EVENT'].concat(__slice.call(_command_spec)));
          return _this;
        },
        social: function(network, action, target, path) {
          var _command_spec;
          if (target == null) {
            target = null;
          }
          ...
          _.trigger.apply(_, ['ANALYTICS_TRACK_SOCIAL'].concat(__slice.call(_command_spec)));
          return _this;
        },
        timing: function(category, variable, time, label, samplerate) {
          var _command_spec;
          if (samplerate == null) {
            samplerate = null;
          }
          ...
          if (samplerate != null) {
            ...
            _command_spec.push(_this.state.config.samplerate);
          }
          _.trigger.apply(_, ['ANALYTICS_TRACK_TIMING'].concat(__slice.call(_command_spec)));
          return _this;
        },
        pageview: function(page_url) {
          ...
          _.trigger.apply(_, ['ANALYTICS_TRACK_PAGEVIEW'].concat(__slice.call(_command_spec)));
          return _this;
        },
        campaign: function(content_key, medium, name, source, term, nokey) {
          if (nokey == null) {
            nokey = null;
          }
        },
        transaction: function(order_id, affiliation, total, tax, shipping, city, state, country) {}
      };
      this.transactions = {
        item: function(order_id, sku, name, category, price, quantity) {},
  ...
  this.__apptools_preinit.abstract_base_classes.push(GoogleAnalytics);

  this.__apptools_preinit.installed_integrations.push(GoogleAnalytics);
  ...
  AppTools = (function() {
    ...
    function AppTools(window) {
      ...
      this.sys.state.core = [CoreModelAPI, CoreAgentAPI, CoreRPCAPI, CoreServicesAPI, CoreUserAPI, CoreRenderAPI];
