...
function sendRequest(path, method, request, onSuccess) {
  ...
  $.ajax({url: path + '.' + method,
          ...
          data: $.toJSON({id: request_id, opts: {}, agent: {}, request: {service: '__meta__', method: method, params: request}}),
          dataType: 'json',
          success: function (response) { onSuccess(response.response.content); },
          error: function (response) { handleRequestError(response.error); }
         });
}
...
function loadServices(when_done) {
  sendRequest(
      ...
      function(response) {
        serviceMap = {};
        console.log("ServiceLayer Response:", response);
